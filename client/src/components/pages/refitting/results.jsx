...
export default function TargetedSequencing({ jobId }) {
  ...
  const loadJobData = async (jobId) => {
    ...
    try {
      ...
      const paramsResponse = await fetch(
        `/mutational-signatures/api/data/input/${jobId}/params.json`
      );
      ...
      const csvResponse = await fetch(
        `/mutational-signatures/api/data/output/${jobId}/H_Burden_est.csv`
      );
      if (!csvResponse.ok) {
        throw new Error(
          `Failed to load CSV data: ${csvResponse.status} ${csvResponse.statusText}`
        );
      }
  ...
  const renderHEstResults = () => {
    ...
    const signatures = [
      ...new Set(results.h_estData.map((row) => row.signature)),
    ].sort();
    ...
    return (
      <div>
        ...
        <Table columns={activityColumns} data={activityData} striped bordered />

        <h3 className="mt-4 h6-title">Burden Values</h3>
        <Table columns={burdenColumns} data={burdenData} striped bordered />
      </div>
    );
  };
  ...
  const renderBurdenEstResults = () => {
    ...
    if (!results.burden_estData || results.burden_estData.length === 0) {
      return (
        <Alert variant="info">
          <strong>No Burden Estimation Data:</strong> This data is not available
          in the current H_Burden_est.csv file. Additional analysis outputs
          would be needed to display burden estimation results.
        </Alert>
      );
    }
    ...
    const columns = [
      ...
      },
      {
        accessor: 'accuracy',
        Header: 'Accuracy',
        Cell: ({ value }) => <span className="text-success">{value}</span>,
      },
    ];

    return (
      <Table columns={columns} data={results.burden_estData} striped bordered />
    );
  };
  ...
  const renderCosineResults = () => {
    ...
    if (!results.cosineData || results.cosineData.length === 0) {
      return (
        <Alert variant="info">
          <strong>No Cosine Similarity Data:</strong> This analysis is not
          available in the current H_Burden_est.csv file. Additional analysis
          outputs would be needed to display cosine similarity results.
        </Alert>
      );
    }
    ...
    return (
      <Table columns={columns} data={results.cosineData} striped bordered />
    );
  };
  ...
  const renderBICResults = () => {
    ...
    if (!results.bicData || results.bicData.length === 0) {
      return (
        <Alert variant="info">
          <strong>No BIC Analysis Data:</strong> This analysis is not available
          in the current H_Burden_est.csv file. Additional analysis outputs
          would be needed to display BIC model comparison results.
        </Alert>
      );
    }
    ...
    return <Table columns={columns} data={results.bicData} striped bordered />;
  };
  ...
  const renderL2NormResults = () => {
    ...
    if (!results.l2NormData || results.l2NormData.length === 0) {
      return (
        <Alert variant="info">
          <strong>No L2 Norm Analysis Data:</strong> This analysis is not
          available in the current H_Burden_est.csv file. Additional analysis
          outputs would be needed to display L2 norm error analysis results.
        </Alert>
      );
    }
    ...
    return (
      <Table columns={columns} data={results.l2NormData} striped bordered />
    );
  };
    ...
    <div className="bg-white border rounded p-3">
      ...
      {!jobId && (
        <Alert variant="warning">
          <strong>No Job Selected:</strong> Please select a job from the Status
          tab to view targeted sequencing results.
        </Alert>
      )}
      ...
      {jobId && !loading && !error && (
        <>
          ...
          >
            <strong>Analysis Complete:</strong> Your refitting analysis has been
          ...
          <Card className="mb-4">
            <Card.Header>
              <h2 className="h5-title mb-0">
                {submittedSignatureType} -{' '}
                {selectedMetric === 'h_est' &&
                  'Refitting Results (H_Burden_est.csv)'}
                {selectedMetric === 'burden_est' &&
                  'Burden_est Table - Mutation Burden Estimation'}
            ...
            <Card.Body>
              {selectedMetric === 'h_est' && <>{renderHEstResults()}</>}

              {selectedMetric === 'burden_est' && (
                <>
                  <p>
                    The Burden_est table shows the estimated mutational burden
                    for each sample compared to the total observed mutations.
                    Accuracy indicates how well the refitting process captured
                    the original mutational burden.
                  </p>
              ...
              {selectedMetric === 'cosine' && (
                <>
                  <p>
                    Cosine similarity measures how well the refitted{' '}
                    {submittedSignatureType} signatures match the original
                    signatures. Values closer to 1.0 indicate better similarity.
                  </p>
                  {renderCosineResults()}
                </>
              )}
              ...
              {selectedMetric === 'bic' && (
                <>
                  <p>
                    BIC (Bayesian Information Criterion) comparison shows model
                    performance for {submittedSignatureType} signatures. Lower
                    BIC scores and negative Δ AIC values indicate better model
                    fit.
                  </p>
                  {renderBICResults()}
                </>
              ...
              {selectedMetric === 'l2norm' && (
                <>
                  <p>
                    L2 Norm analysis shows the reconstruction error for each
                    sample using {submittedSignatureType} signatures. Lower
                    values and positive improvements indicate better refitting
                    performance.
                  </p>
                  {renderL2NormResults()}
                </>
