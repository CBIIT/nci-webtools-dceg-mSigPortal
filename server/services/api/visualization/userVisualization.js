  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      try {
        ...
        console.error('Error parsing matrix data. ');
        reject(error);
      }
    } catch (error) {
      reject(error);
...
async function submitVisualization(req, res, next) {
  ...
  try {
    ...
    const params = {
      ...req.body,
      Input_Path: path.resolve(env.INPUT_FOLDER, userId, req.body.Input_Path),
      Output_Dir: path.resolve(env.OUTPUT_FOLDER, userId),
      ...(req.body?.Bed && {
        Bed: path.resolve(env.INPUT_FOLDER, userId, req.body.Bed),
      }),
    };
